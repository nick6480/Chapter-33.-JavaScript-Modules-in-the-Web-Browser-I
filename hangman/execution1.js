const isValid = function(e) {
    ...
    let time = document.formal.time;

    if (name.value === "" || time.value === "") { // hvis born eller name ikke er udfyldt, kan det ikke submittes //
        window.alert("Both fields must be filled in");
        name.focus();
        e.preventDefault(); // preventer submit //
        return false;
    } else {
        createCookie(name.value, time.value, 0.00094444); // name.value = navnet angivet, born = datoen, 42 = værdien, 0.00.. = hvor lang tid cookien er aktiveret //
        return true;
    }
}

const getstarted = function() {
    console.log(`Cookie name=score? ${readCookie('name=time')}`);
